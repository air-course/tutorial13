...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{itemize}
		\vspace{0.2cm}
		\item The covariance is defined over an \textbf{error (perturbation) state} $\delta x$:
		\[
		\delta x =
		\left[
		\delta\theta,\;
		...
		\item \textbf{Error-state EKF}: the filter estimates a small correction $\delta x$ and injects it into the state, ensuring consistent handling of orientation on $\mathrm{SO}(3)$.
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Process Jacobian} $F_k = \left.\frac{\partial f}{\partial x}\right|_{\hat{x}_{k-1}}$ used to propagate the error state $\delta x_k \approx F_k \delta x_{k-1}$:
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Measurement Jacobian} $H_k = \left.\frac{\partial h}{\partial x}\right|_{\hat{x}_k^-}$ maps the error state to the residual $r_k \approx H_k \delta x_k$:
		\[
		H =
		\begin{bmatrix}
		...
		\item For robustness, set $H_\theta = 0$ on real data. Position terms still constrain the base position:
		\[
		H_p = I,\quad H_{p_F} = -I
		\]
	\end{itemize}
